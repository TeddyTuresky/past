% This script produces a spreadsheet describing the mean and max discplacements
% from the origin and inter-scan movements.
% It will ask you to select the number of the 1st scan. For instance if you
% exclude the 1st 3 scans , you would input 4
...
input = spm_select(Inf,'dir','Select Directories'); 
depdir = '/Volumes/FunTown/allAnalyses/iFCcompAnalysis/';
...
for i=1:size(filenames,1);
         ...
         g=0;
                ...
                % remove any elements in which delta is greater than 0.75mm
                art=load([trim_filename '/BadScanRegressors_1.5perc_0.75mm.txt']);
                indx = find(art);
...
writetable(T,[depdir 'mot_desc_art_dys.csv']);